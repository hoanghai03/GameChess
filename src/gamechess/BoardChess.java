...
public class BoardChess implements Serializable {
    ...
    public static JPanel paintChessBox(Piece pieceSelected,
        ...
        if (BoardChess.turn == Piece.Black) {
            Main.getInstance().lb_turn.setText("Turn of Black");
        }
        if (BoardChess.turn == Piece.White) {
            Main.getInstance().lb_turn.setText("Turn of White");
        }
        ...
        for (int iRow = 1; iRow <= 8; iRow++) {
            for (int iCol = 1; iCol <= 8; iCol++) {
                ...
                // hien thi cac o ma quan co co the di
                if (listPieceGo != null) {
                ...
                if (BoardChess.chessBoard[iRow - 1][iCol - 1].getColor() != -1) {
                    ImageIcon icon = icon = new ImageIcon("image/"
    ...
    // kiem tra ket qua:  1 - win, 2 - hoa, 0 - thua
    ...
    public static boolean breakSecret() {
        ...
        // lay tat cac cac buoc co the di cua nguoi choi
        ...
        for (int i = 0; i < listChessAndCoordiante.size(); i++) {
            ...
            for (int j = 0; j < listChessAndCoordiante.get(i).getListCoordinate().
                ...
                BoardChess.chessBoard[iRowBefore][iColBefore] = new EmptyCell(-1, new Coordinate(iRowBefore, iColBefore));
        ...
        for (int iRow = 0; iRow < 8; iRow++) {
            for (int iCol = 0; iCol < 8; iCol++) {
                if (BoardChess.chessBoard[iRow][iCol].getColor() == BoardChess.turn) {
                    ...
                    ChessAndCoordinare temp = new ChessAndCoordinare(BoardChess.chessBoard[iRow][iCol].
    ...
    // khoi tao luot di hien tai la trang va kiem tra da khoi tao hay chua
